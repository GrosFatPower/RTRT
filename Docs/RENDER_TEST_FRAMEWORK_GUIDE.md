...
`software_optimized` enables the retained optimized software paths. `software_simd` explicitly selects the available platform SIMD path. `software_fallback` keeps tiled rendering but disables compact hits and direct color writes, allowing those paths to share a visual baseline. `transparency` controls blended geometry; alpha-masked materials remain active when it is disabled. Deferred cases also accept `refraction`, `refraction_max_steps`, `refraction_step_size`, `refraction_max_distance`, `refraction_thickness`, and `refraction_edge_fade`.
