...
Supported optional test fields are `resolution`, `frames`, `thresholds`, `environment_map`, `camera`, and `baseline`. Resolution values and frame counts must be positive. A camera requires three-number `position` and `pivot` arrays plus a positive `fov`; `near` is positive and `far` must be greater than `near`.
