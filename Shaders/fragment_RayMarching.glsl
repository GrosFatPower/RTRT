...
void mainImage( out vec4 color, in vec2 coord )
{
    ...
    for( int n=ZERO; n<AA; n++ )
    {
        ...
        vec2 p = (2.0*(coord+o)-u_Resolution.xy)/u_Resolution.y;
#else    
        vec2 p = (2.0*coord-u_Resolution.xy)/u_Resolution.y;
#endif
        ...
        vec3 rd = ca * normalize( vec3(p,fl) );
         ...
         // ray differentials
        vec2 px = (2.0*(coord+vec2(1.0,0.0))-u_Resolution.xy)/u_Resolution.y;
        vec2 py = (2.0*(coord+vec2(0.0,1.0))-u_Resolution.xy)/u_Resolution.y;
