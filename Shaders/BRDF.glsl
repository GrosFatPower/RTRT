...
// ----------------------------------------------------------------------------
float D_GGX( in float iAlpha, in float iNdotH )
{
...
// ----------------------------------------------------------------------------
float G_Smith( in float iAlpha, in float iNdotV, in float iNdotL )
{
...
// ----------------------------------------------------------------------------
vec3 FSchlick( in vec3 iF0, in float iVdotH )
{
...
vec3 BRDF( in vec3 iN, in vec3 iV, in vec3 iL, in vec3 iF0, in Material iMat )
{
  ...
  vec3  F = FSchlick(iF0, VdotH);         // Fresnel reflectance (Schlick approximation)
  float D = D_GGX(alpha, NdotH);          // Normal distribution function
  float G = G_Smith(alpha, NdotV, NdotL); // Geometry term
