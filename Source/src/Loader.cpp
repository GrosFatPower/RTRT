...
bool Loader::LoadShape( const std::string & iFilename, Shape * oShape )
{
  oShape = nullptr;
  ...
  if (!ret)
  {
    std::cout << "Shape : Unable to load object" << iFilename << std::endl;
    if ( !err.empty() )
      std::cout << err;
    if ( !warn.empty() )
      std::cout << warn;
  ...
  {
    std::cout << "Shape : loading object" << iFilename << " ..."<< std::endl;
    std::cout << warn;
  }

  oShape = new Shape;

  oShape -> _Filename = iFilename;
  ...
  for ( unsigned int i = 0; i < nbVertices; ++i )
  {
    ...
    oShape -> _Vertices.push_back(coords);
  }
  ...
  for ( unsigned int i = 0; i < nbNormals; ++i )
  {
    ...
    oShape -> _Normals.push_back(normal);
  }
  ...
  for ( size_t s = 0; s < shapes.size(); ++s )
  {
    ...
    for ( size_t f = 0; f < shape.mesh.num_face_vertices.size(); ++f )
    {
      for ( size_t v = 0; v < 3; ++v )
      {
        ...
        oShape -> _Indices.push_back(indices);
      }
      index_offset += 3;
    }
  }
  oShape -> _NbFaces = (int) oShape -> _Indices.size();
